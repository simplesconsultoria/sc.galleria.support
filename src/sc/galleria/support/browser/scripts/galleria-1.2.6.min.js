...
bind:function(elem,value){ if(Galleria.TOUCH){return;}
...
}else if(optval==='empty'||optval==='numbers'){thumb={container:Utils.create('galleria-image'),image:Utils.create('img','span'),ready:true}; if(optval==='numbers'){$(thumb.image).text(i+1);}
...
if(rel){data.big=rel;} 
self._data.push($.extend({title:img.attr('title')||'',thumb:img.attr('src'),image:img.attr('src'),big:img.attr('src'),description:img.attr('alt')||'',link:img.attr('longdesc'),original:img.get(0)
},data,config(img)));}); if(this.getDataLength()){this.trigger(Galleria.DATA);}else{Galleria.raise('Load failed: no data found.');}
